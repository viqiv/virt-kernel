use core::{arch::asm, cmp::max, hint::spin_loop, ops::BitOr, ptr::NonNull};
...
pub fn init(regs: &mut Regs, irq: u32) {
    ...
    let root = FILES.as_ref()[0].acquire();
    root.as_mut().fid = 0;
    root.as_mut().qid = p9.qid;
    root.as_mut().iou = u16::MAX as u32;
...
pub fn open(path: &str, mode: u32) -> Result<File, ()> {
    if let Ok((fid, _)) = ops::walk(path) {
        if let Ok((qid, iou)) = ops::open(fid, mode) {
            return Ok(File { fid, iou, qid });
        } else {
            return Err(());
        }
    };
...
static FILES: SyncUnsafeCell<[Lock<File>; 128]> = SyncUnsafeCell::new([
    Lock::new("9", File::zeroed()),
    Lock::new("9", File::zeroed()),
    Lock::new("9", File::zeroed()),
    Lock::new("9", File::zeroed()),
    Lock::new("9", File::zeroed()),
